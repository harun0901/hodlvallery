// import { blockQuery, blocksQuery, getApollo, latestBlockQuery } from "../../core/index";
import { getApollo } from "../apollo";
import { latestBlockQuery } from "../queries/blocks";
...
export async function getOneDayBlock(client = getApollo()) {
  const date: any = startOfMinute(subDays(Date.now(), 1));
...
export async function getTwoDayBlock(client = getApollo()) {
  const date: any = startOfMinute(subDays(Date.now(), 2));
  const start = Math.floor(date / 1000);
  const end = Math.floor(date / 1000) + 600;
...
export async function getSevenDayBlock(client = getApollo()) {
  const date: any = startOfMinute(subWeeks(Date.now(), 1));
  const start = Math.floor(date / 1000);
  const end = Math.floor(date / 1000) + 600;
