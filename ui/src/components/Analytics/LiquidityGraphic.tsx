...
interface LiquidityGraphicProps {
  data: Array<any>
}
const LiquidityGraphic: React.FC<LiquidityGraphicProps> = ({data}) => {
    ...
    return (
        <div className={'w-full h-full rounded-55 bg-gray-200 border-10 border-white relative overflow-hidden'}>
          <div className={'w-full pt-25px pl-28px pr-10px'}>
            <p className={'font-bold text-base text-gray-400 uppercase'}>Liquidity</p>
          ...
          <div className={'absolute -bottom-5px left-0 w-full'}>
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={setGraphRange(data)}>
                <defs>
                ...
                <Area type="monotone" dataKey="pv" stroke={'none'} dot={{r: 0}} fillOpacity={1} fill="url(#colorPv)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
...
export default LiquidityGraphic
