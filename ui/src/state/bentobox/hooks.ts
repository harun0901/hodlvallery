  ...
  return useMemo(() => {
    ...
    return tokenAddresses
      .map((key: string, i: number) => {
        ...
        const usd = BigNumber.from(balanceData.result[0][i].rate).isZero()
          ? BigNumber.from(0)
          : BigNumber.from(e10(token.decimals))
            .mul(BigNumber.from(uiData.result[0].ethRate))
            .div(BigNumber.from(balanceData.result[0][i].rate))
