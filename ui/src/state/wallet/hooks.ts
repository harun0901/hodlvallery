...
export function useBTCBalance() {
    ...
    const {activeWalletDetails: {unitNetworkBalance}} = useCrossChainClient()
    return useMemo(()=>{
        return CurrencyAmount.fromRawAmount(Bitcoin.onChain(chainId), JSBI.BigInt(unitNetworkBalance.toString()))
    },[])
}
/**
...
export function useCurrencyBalances(
  ...
  const containsBTC: boolean = useMemo(() => currencies?.some((currency) => currency?.isNative && currency?.symbol?.toUpperCase() === 'BTC') ?? false, [currencies])
  ...
  return useMemo(
    () =>
      currencies?.map((currency) => {
        ...
        if (currency.isNative && currency.symbol?.toUpperCase() === 'ETH') return ethBalance[account]
