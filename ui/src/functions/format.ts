...
export const limitNumber = (x: number, main = 9, precision = 9): string => {
  let numStr = x.toString();
  if (numStr.includes(".")) {
    const parts = numStr.split(".");
    numStr = parts[0].slice(0, main) + "." + parts[1].slice(0, precision);
  } else {
    numStr = numStr.slice(0, main);
  }
  return numStr;
};

export const formatToCurrencySystem = (labelValue: string | number): string | number => {
