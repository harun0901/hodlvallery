...
export function getUSDString(amount: BigNumberish, token: any): string {
  const usd = BigNumber.from(amount)
    .mul(token.usd)
    .div(e10(token?.decimals ? token.decimals : token.tokenInfo.decimals))
  return parseFloat(usd.toString())
...
export function easyAmount(
  ...
  return {
    value: amount,
    string: parseFloat(amount.toString()).toFixed(token?.decimals ? token.decimals : token.tokenInfo.decimals),
